...
--   - record_photos は owner_id を持たないため、親 daycare_records の household を継承して
...
--   *_household ポリシーのみを drop する（owner_id ポリシーは残す）。各 create policy に
...
create policy "feedback_insert_member"
  ...
  with check (public.has_household_role(household_id));
