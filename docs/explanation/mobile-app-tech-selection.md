...
| テスト基盤 | Playwright E2E 13 spec ＋ VRT（D25 / D30 / D31） | ブラウザ前提。ネイティブ書き直しで**丸ごと失効**する資産 |
| 運用制約 | Vercel Hobby ＋ Supabase Free の**無料枠で完結**（D18） | 金銭コストの許容度が低い |
| 利用者 | 家族数人（ストアでの発見・獲得は価値ゼロ） | 「ストア公開」自体が目的になっていないか要確認 |

**すでに満たしていること**も明確にしておく。現状の PWA で、iOS / Android とも
**ホーム画面インストール・スタンドアロン起動・オフライン表示・Web Push（iOS は 16.4+ / EU 圏を除く）**
は動く。つまり「アプリ化」で**増える**ものだけが検討対象になる。
...
| **C6** | **既存 CI / テスト基盤との整合** | 10 | Playwright の E2E（D25）・VRT（D31）・スクリーンショット生成（D26）が生きるか | D25〜D31 で積んだ基盤。失効すると品質ゲートを一から作り直す |
| **C7** | **セキュリティモデルとの整合** | 10 | Cookie セッション（`@supabase/ssr`）・OAuth リダイレクト・RLS 依存度の変化・リモートコード実行の有無 | RLS が一次防衛線。クライアント直叩きが増えると**アプリ層の認可ヘルパー（D16）が消える** |
...
| C6 CI 整合 (10) | 5 | 4 | 3 | 1 | 1 | 1 | 4 |
| C7 セキュリティ (10) | 5 | 5 | 3 | 4 | 4 | 4 | 2 |
| **合計** | **91** | **79** | **61** | **58** | **54** | **54** | *(失格)* |
...
**EU 圏では DMA 対応でスタンドアロン PWA が外され Web Push が効かない**、バックグラウンド処理が乏しい。
...
**C（61 点）— Capacitor ＋ 静的エクスポート（アプリ化する場合の本命）.**
...
- **C7**: 上記の書き換えは「**アプリ層の認可ヘルパーが消えて、RLS 一枚に依存する**」ことを意味する。
  CLAUDE.md が「一次防衛線は RLS」としているので致命傷ではないが、**D16 で 2 度取り違えたあの認可判断
  （更新/削除は対象行の `household_id` で判定）を RLS 側だけで担保しきれているか**の検証が前提条件になる。
  D30 の Integration 層（「どの認可ヘルパーを呼んだか」を見るテスト）は**検証対象ごと無くなる**。
...
Playwright 基盤の失効（C6）** が同時に来るため。**React と TypeScript と Supabase クライアントの知識は再利用できる**
（そこが F・G との差）が、**コードは再利用できない**。

**F（54 点）— Flutter.** E の欠点をすべて持ち、さらに **Dart なので TypeScript の型資産（`src/types/database.ts`）も
Supabase の TS クライアントも捨てる**。E を上回る理由がこのプロジェクトには無い。

**G（54 点）— フルネイティブ.** 得られるものは最大だが、**web を含めて 3 コードベース**になる。
...
1. **「アプリ化」で増える差分が小さい。** ホーム画面起動・オフライン・Web Push（iOS 16.4+）は**もう動いている**。
...
- Web Push を入れるかは別途判断（iOS は 16.4+ かつ**ホーム画面インストール済み**が条件、**EU 圏では不可**）。
...
- **カメラ・共有シートからの記録投入**（他アプリの写真を mfmf へ「共有」で送る）が欲しくなった。
- 家族に **EU 圏の利用者**が出た（＝ iOS の PWA が Safari タブに落ちて通知が消える）。
- **iOS の PWA サポートが後退**した（Apple の方針変更は前例がある）。
...
| iOS の PWA 制約（16.4+ の Web Push、EU での不可、iOS 26 の既定変更） | [MagicBell — PWA iOS Limitations (2026)](https://www.magicbell.com/blog/pwa-ios-limitations-safari-support-complete-guide) / [MobiLoud](https://www.mobiloud.com/blog/progressive-web-apps-ios) |
