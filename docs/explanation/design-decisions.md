...
- **将来 (B) 世帯共有へ移行する場合**は `household_id` 列を追加して移行する想定。
  具体的な分割・順序は **Phase 3.5**（エピック [#91](https://github.com/godhuu0505/mfmf/issues/91)、
  S1 テナント基盤 [#92](https://github.com/godhuu0505/mfmf/issues/92)）で計画済み。`owner_id` ポリシーと
  併存させた無停止移行（新旧 RLS 併存 → pgTAP で検証 → 切替）を前提とし、現行の (A) を直接弱めない。
