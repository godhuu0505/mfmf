...
つまり検討対象は「アプリ化で**増える**もの」と「**どの案でも払うが、量は等しくない**通知の実装コスト」
（A・B は Web Push、C・E・F・G は APNs / FCM で後者が重い）の 2 つに分かれる。
...
> **「オフライン」は C5 の中で案ごとに見る**（独立観点にはしないが、「差が小さい」わけではない）——
> A・B・C は Service Worker / バンドル済みアセットで `/offline` を出せる。**D は `server.errorPath`
> でローカルのエラーページを出せるので「必ずホワイトアウトする」わけではない**が、
> **リモート URL を読む構成上、オフラインでは認証済みの記録そのものを表示できない**
> （degraded な案内画面止まり）。E・F・G はネイティブ側の実装次第。
> 「パフォーマンス」は Supabase 往復が支配的で候補間の差が小さいため、観点として立てていない。
...
### 4-1b. ストアに出すなら案によらず必要になるもの（当初見落としていた）

**ストアに出すと発生する前提**が 2 つある（かかる範囲は下表のとおり違う）。

- **Sign in with Apple（iOS / ガイドライン 4.8）** —— サードパーティのログインを提供するアプリは、
  **同等のプライバシー保護オプションを併せて提供する**必要がある。mfmf は `/login` で
  **Google ログインを出している**が（`src/app/login/LoginForm.tsx:55`）、**Apple 相当の導線は無い**
  （リポジトリ全体に実装なし）。iOS に出す案（C・D・E・F・G）は、**プロバイダ設定・コールバック・UI の
  追加が前提条件**になる。
- **アカウント削除の導線（Apple / Google Play 双方）** —— アプリ内でアカウントを作れるなら、
  **アプリ内から削除できる経路**が要る。mfmf はセルフサインアップと OAuth 作成があるが、
  設定画面には「**世帯・アカウントを削除する導線は準備中です（#51）**」とあり
  （`src/app/(app)/settings/page.tsx`）、**Supabase Auth のユーザーを消す実装は存在しない**。
  ストアに出す全案（B〜G）で、**破壊的なバックエンド処理と UI の新規実装**が前提になる。
  既存の `deleteHousehold` は**参照データの無い世帯だけ**が対象で、これの代わりにはならない。

**かかる範囲は 2 つで違う。**

| 前提 | かかる案 | かからない案 |
| --- | --- | --- |
| Sign in with Apple | **C・D・E・F・G**（iOS に出す案） | **A**（ストアに出さない）／**B**（Android のみ） |
| アカウント削除の導線 | **B・C・D・E・F・G**（ストアに出す全案） | **A** |

つまり **B は削除導線だけで済み、SIWA は要らない**。これは B の小さな相対優位だが、
**B は既にストア案の中で最上位（70）**なので順位は動かない。
**A はどちらも払わなくてよい**ので、**A と他案の差は当初の見積もりより広い**。
「ストアに出す」と決めた時点で、**機能追加ゼロの実装が最低 1 件、iOS を含むなら 2 件確定する**。
...
> 版がズレて壊れうる**ので当てにしきれず、**オフラインでは（`server.errorPath` で案内は出せても）
> 記録そのものを見られない**ため。
> **「プラグインが動かない」と断定するのは誤り**で、正しくは**条件付きの版ズレリスク**。**C7 = 2** は、ネイティブシェルが**リモートの内容を読み込む**
> 形になり、プラグインと web のバージョン不整合も攻撃面・障害面を広げるため。
> **C2 が 5 でなく 4 なのは、「書き換えゼロ」が正確ではないため** ——
> `LoginForm.tsx:55` の `signInWithOAuth` は**現在のユーザーエージェントをリダイレクトする**が、
> D ではそれが**埋め込み WebView** になる。**Google は OAuth で埋め込みユーザーエージェントを禁止**
> しているので、**システムブラウザ経由のサインインと検証済みコールバック**を別途作る必要がある。
> **合計 57 で E（60）を下回る**。「書き換えずに済む」という長所自体が、認証まで含めると成り立たない。

> **C（63）・E（60）・D（57）は 6 点の幅に収まっている**ので、C5 / C6 / C7 の付け方で順位が入れ替わりうる。
...
**オフラインでは記録を見られず**（`server.errorPath` で案内画面は出せるが、
A のような閲覧の継続にはならない）、Apple 4.2 の *lazy wrapper* 類型にも当たりやすい（C1 = 2）。
...
「審査を完全に回避できる」のは **internal（＋コンソールアカウント配布）／Ad Hoc（＋ UDID 管理）／
無料 Personal Team（＋週 1 の再署名）** の 3 つで、**いずれも C1 で浮いたぶんを C3 の恒久運用で払い直す**構造になっている。
