...
- **理由**: 「いつ・どこへ出すか」を Actions に一本化し、main へのマージで即本番に出てしまう事故を防ぐ。
- main へのマージは **Preview 止まり**、**本番反映はタグ付き Release の公開がトリガ**という二段構え。
- **DB マイグレーションも CI/CD で自動適用**する（`supabase db push`）。Vercel deploy より前に
  `migrate` ジョブを走らせ、本番 DB とコードのスキーマ整合性を保つ。
- preview と production は **同一 Supabase プロジェクトを共有**する（Free 枠の active 2 プロジェクト
  上限のため、preview 用 DB を別建てすると本番運用と両立できない）。Database Branching は Pro プラン
  以上の機能で、現スコープでは採用しない。
