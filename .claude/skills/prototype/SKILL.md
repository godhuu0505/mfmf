...
> ⚠️ **E2E の基盤（Playwright）はまだ入っていません。** `package.json` に test スクリプトが無く、
> CI も lint / typecheck / build ＋ pgTAP だけです。**最初にこの手順を通す機能で、Playwright を
> 1 本だけ入れてください**（D25 の受け入れ判定と、D26 の画面スクリーンショット生成は
> **同じ基盤に乗ります** —— 1 つ入れれば両方が動き出す）。入れたらこの注記を消してください。
