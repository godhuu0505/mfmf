...
> A・B・C は Service Worker / バンドル済みアセットで `/offline` を出せるが、**D はリモート URL を
> 読むだけなのでホワイトアウトする**。E・F・G はネイティブ側の実装次第。
...
**B〜G に共通してかかる前提**が 2 つある。**A（ストアに出さない）だけが免除される。**
...
**この 2 つは B〜G に等しくかかるので相対順位は動かさない**（通知と同じ扱い）。
ただし **A だけが払わなくてよいコスト**なので、**A と他案の差は当初の見積もりより広い**。
「ストアに出す」と決めた時点で、**機能追加ゼロの実装が 2 件確定する**ことは記録しておく。
