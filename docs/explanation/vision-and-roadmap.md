...
> `has_family_role()` に加え、**ゲスト用の付与テーブル（期限・対象付き）** を別途参照する設計を想定。
...
```
families         (id, name)
family_members   (family_id, user_id, role)            -- 内部メンバーの多対多
guest_grants     (family_id, user_id, scope_pet_id,    -- 外部ゲストの期間/対象限定アクセス
                  role, valid_from, valid_to)
pets             (id, family_id, name, species, birthday, avatar_path)
daycare_records  (… pet_id を追加。owner_id は移行期間中"残す"＝無停止移行)
record_photos    (… drive_file_id + thumb_path)        -- 原本Drive・サムネSupabase
events           (id, pet_id, kind, start_at, end_at,  -- 予定・予防接種・通院
                  google_event_id, remind_at)
invitations      (id, family_id, email/code, role, expires)
```

- **多対多**：`family_members` でユーザーは複数家族に所属可。`pets.family_id` で家族は複数ペットを保有。
- **「1匹を複数家族で共同管理」** まで踏み込むなら `pets` も junction 化（`pet_families`）して "多々々" に拡張可能（やりすぎ注意点）。
...
| --- | --- | --- |
| **Drive** | 写真原本の保管庫（15GB 枠）。サムネのみ Supabase | 採用（Phase 3） |
| **Calendar** | 予定・予防接種・通院を家族カレンダーへ双方向同期 | 採用（Phase 4） |
| **OAuth (ログイン)** | 家族・外部が各自アカウントで参加 | 採用（Phase 1） |
...
> トークン管理が増える点を Phase 4 着手前に別途設計する。
...
| Phase | テーマ | 主な内容 |
| --- | --- | --- |
| **0** | 足場固め | `deletePhoto` 認可修正・**エクスポート**・孤児ファイル掃除 |
| **1** | 家族とログイン | Google ログイン、`families/family_members/pets`、`has_family_role()`、既存データ移行、前回値プリフィル |
| **1.5** | 体験 quick win | 定型テンプレ・「○年前の今日」・音声入力・全文検索・カレンダー表示 |
| **2** | 権限と招待 | 3段階 role の RLS、招待フロー、**外部ゲスト（保育園/シッター）**、viewer UI |
| **3** | 写真を Drive へ | Drive OAuth（原本）＋サムネ Supabase、アルバム表示 |
| **4** | 予定とリマインダー | Google Calendar 同期、予防接種・通院リマインダー、日程調整 |
| **5** | レポートとハブ統合 | 期間/共有レポート、Google サービス横断 UI、月次ダイジェスト |
