...
> **ただし公式の `postgres:16` イメージに pgTAP は入っていません。** 残す 5 ファイルも冒頭で
...
パスのスコープが同じ `household_id` でも、**必要な role が違う**からです。

| 操作 | いま要求している権限 | 根拠 |
| --- | --- | --- |
| ユーザー自身のアバター | 本人（`owner_id` 一致） | `settings/actions.ts` |
| **世帯のアバター** | **owner のみ** | `requireOwnerOf(householdId)`（`settings/actions.ts:179`） |
| **ペットのアバター** | **editor 以上**（viewer 不可） | `canEdit(role)`（`pets/actions.ts:120`） |

**「世帯のメンバーか」だけで署名すると、viewer に世帯アバターの書き込みを許すことになります。**
