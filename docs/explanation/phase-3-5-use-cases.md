...
- **S2 RBAC（[#46](https://github.com/godhuu0505/mfmf/issues/46)）**: **DB/RLS は完了**。role は CHECK 制約（owner/editor/viewer, `20260703170000`）、write は editor 以上に限定、旧 owner_id write ポリシーは切替済み（drop, `20260704000000`。feedback 送信・タグ昇格用 update 等の意図した例外は migration ヘッダー参照）。ロール変更/メンバー削除/退出/世帯名変更の RLS と D6（最後の owner 保護）トリガも導入済み。**残**: Server Action のロール検査・UI のロール別出し分け・メンバー管理 UI（UC-A06/H03 の画面）。
