...
- Supabase の `auth.users` **と `auth.identities`** を書き出し、**`uid` を指定したまま**
  ...
  **Google の初回ログインをリハーサルしてから** UID 保存を確定と見なすこと。
  ...
  ただし **ハッシュを配列に入れれば済む話ではありません** —— インポートには
  **アルゴリズム（BCRYPT）とコスト（ラウンド数）を明示する必要があり、SDK は
  ハッシュ文字列からそれを推測しません**。**コストの異なるユーザーを 1 バッチに混ぜると
  失敗するか、通っても検証できないパスワードになります**（＝「再設定不要」が嘘になる）。
  既存ハッシュからコストを読み取り、**必要ならコストごとにバッチを分ける**こと。
  **email/password でのログインを必ずリハーサルに含める**
