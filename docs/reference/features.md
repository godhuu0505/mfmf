...
| ストレージ命名規約 | `{household_id}/{record_id}/{filename}`（未所属時・既存分は `{owner_id}/...`。`src/lib/storagePath.ts` で生成・検証） |
...
| 機能 | 概要 |
| --- | --- |
| 自由タグ付け 🔑 | 世帯で共有する辞書から既存タグを選択、または新規追加（フォーム上で補完） |
| タグ一覧表示 | 記録カード上にタグを表示 |
...
匿名共有リンク（旧 `share_links`）は Phase 3.5 S4 で廃止し、フォローアップ（#117）で
テーブル・`get_shared_view` を物理削除した。閲覧共有はアカウント招待へ一本化。

| 機能 | 概要 | 画面 |
| --- | --- | --- |
| viewer 招待 🔑 | 家族などを閲覧のみで招待（世帯の記録すべて） | `/settings` |
| ゲスト招待 🔑 | 保育園・シッターを対象ペット・期間限定で招待（写真は非公開＝D8） | `/settings` |
| 招待の取消 🔑 | いつでも失効。取消で相手はアクセスを失う | `/settings` |
| 旧リンクの終了案内 🌐 | 旧トークンの受け皿。DB 参照なしの静的案内・`robots: noindex` | `/shares`, `/share/[token]` |
