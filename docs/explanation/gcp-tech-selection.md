...
| **金銭コスト（TCO）** | **15%** | D18 の中心。ただし**額そのものより「上限で止まる」か「使った分だけ請求される」かの性質**が重要 |
...
| F. GCE 1 台に全部載せ | 3 | 3 | 1 | 5 | 1 | 2 | **52** | **②③ で落ちる** |
...
### 「GCP に無料の Postgres は無い」は不正確だった

案 F を真面目に検討して分かりました。**Always Free の e2-micro が 1 台あります。**
ただし **us-west1 / us-central1 / us-east1 限定**で、**Asia を選んだ時点で普通に課金されます**
（[Always Free の条件](https://agentdeals.dev/gcp-free-tier-2026)）。つまり選べるのは
**「月 ¥0・日本から往復 100ms 超・単一 VM・バックアップ自作・OS のパッチも自分」**という束です。

これは**金銭コストを人の時間と信頼性で買う**取引で、重み付け（運用 20% / 信頼性 10%）では
負けますが、**「¥0 でなければ移行しない」なら唯一の選択肢**です。正直に机の上に載せておきます。
...
2. **月 ¥0 を絶対条件にするか** —— するなら案 F（US リージョンの単一 VM）しかありません。
   移行案が前提にしていた「月 ¥1,800」は、**この束を買わない代わりに払う額**です。
