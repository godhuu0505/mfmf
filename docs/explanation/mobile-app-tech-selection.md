...
> ネイティブシェルと一致していれば bridge は生きる）、**web を配信し直すと古いシェルと
> 版がズレて壊れうる**ので当てにしきれず、オフライン時はホワイトアウトするため。
> **「プラグインが動かない」と断定するのは誤り**で、正しくは**条件付きの版ズレリスク**。**C7 = 2** は、ネイティブシェルが**リモートの内容を読み込む**
> 形になり、プラグインと web のバージョン不整合も攻撃面・障害面を広げるため。
> **合計 61 で C（63）と E（60）の間**。「書き換えずに済む」という一点で買えるものは、
> この重み付けでは A・B・C を超えない。

> **C（63）・D（61）・E（60）は 3 点の中に並んでいる**ので、C5 / C6 / C7 の付け方で順位がひっくり返りうる。
...
**D（61 点）— Capacitor ＋ `server.url`.**
**唯一「書き換えゼロ」で Android / iOS に出せる案**（hosted な Next.js をそのまま読むので
Server Action 30 本が生き残る）。それでも中位に留まるのは、その一点以外がすべて悪いから ——
Ionic 自身が本番用途を否定しており、**web を配信し直すたびにネイティブシェルと
バージョンがズレて壊れうる**（C3 = 2）。プラグインは**呼べないわけではない**が
この版ズレのせいで当てにしきれず（C5 = 3）、
...
つまり**家族全員に開発者コンソールのアカウントを配ることになる**。選択肢は 4 つ：
...
| **無料の Personal Team**（Xcode） | 端末 3 台程度 | なし | **`$99/年` が要らない唯一の道**。ただし**プロビジョニングが約 7 日で失効**し、そのたびに Mac に繋いで署名し直す必要がある。家族の端末を毎週回ることになるので、実運用にはまず耐えない |
...
**TestFlight と Ad Hoc は `$99/年` が要る**（無料の Personal Team だけが例外だが、
**7 日ごとの再署名**という代償が大きすぎて家族運用には向かない）。さらに
**TestFlight は 90 日でビルドが失効**し、**Ad Hoc は年 1 回の再配布・再インストール**が要る
—— **どの道も「置いたら終わり」にはならず、失効の周期が違うだけ**。
