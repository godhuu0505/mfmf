...
- **S4 外部ゲスト（[#93](https://github.com/godhuu0505/mfmf/issues/93)）**: **着手中（DB 基盤導入済み）**。`guest_grants`（対象ペット・期間限定、`20260705000000`）+ ゲスト RLS ヘルパー（`has_guest_access` / `has_guest_record_access`）+ 記録の明示共有フラグ `daycare_records.guest_visible`（D8 既定 deny）。可視・書込・失効・残置・自己昇格不可は pgTAP（`guest_grants_test.sql` 28 assert）で担保。招待導線（S3 流用）・ゲスト UI・`share_links` 廃止（D4: S4 完了時に即廃止）は後続 PR。
